...
argparser.add_argument('--model_dir',default='./outputs/model', help='path to model dir')
...
def predict(args: argparse.Namespace):
  ...
  ### load raw test amr
  amr_test_raw = utils.load_amr_dataset(args.data_dir, 'test')
  ...
  ### vectorized AMR test
  amr_test = utils.AMR('test', amr_test_raw, sent_vocab, tree_vocab)
  ...
  ### prediction
  preds = []
  for i in tqdm.trange(len(amr_test), desc=f'[Inference]'):
    sent, tree = utils.to_device(amr_test[i], device)
  ...
  # export
  preds_path = os.path.join(args.model_dir, C.GRAPH_PRED_FORMATTER.format('test'))
  print("Exporting test predictions at", preds_path)
  utils.export_txt(preds, preds_path)

  # bleu
  bleu = utils.raw_corpus_bleu(preds, amr_test_raw[1])
