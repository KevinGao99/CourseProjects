...
path_args.add_argument(
  '--data_dir', default='./outputs/processed_data', help='path to processed amr data directory')
path_args.add_argument(
  '--model_dir', default='./outputs/model', help='path to model outputs')
path_args.add_argument(
  '--glove_dir', help='path to glove dir; should be specified when using GloVe')
...
model_args.add_argument(
  ...
  choices=['seq2seq', 'bahdanau', 'luong_dot', 'luong_general'])
model_args.add_argument(
  '--embed_dim', type=int, default=100, help='embedding dimension')
model_args.add_argument(
  '--rnn', type=str.lower, default='gru', choices=['gru', 'lstm'], # TODO: change this to GRU befor release
...
exp_args.add_argument(
  '--epochs', type=int, default=100, help='number of training iterations')
exp_args.add_argument(
...
def train(args: argparse.Namespace):
  ...
  ### load raw amr
  amr_train_raw = utils.load_amr_dataset(args.data_dir, 'train')
  amr_dev_raw = utils.load_amr_dataset(args.data_dir, 'dev')

  ### load vocab
  sent_vocab, tree_vocab = utils.load_vocabs(args.data_dir)
  sent_itos, sent_stoi = sent_vocab
  tree_itos, tree_stoi = tree_vocab
  print(f"\nVocab Info:\n Sent: {len(sent_itos)}\n Tree: {len(tree_itos)} => {', '.join(tree_itos[:10])}")

  utils.validate_vocab(sent_stoi)
  utils.validate_vocab(tree_stoi, is_target=True)

  ### vectorized dataset
  amr_dev = utils.AMR('dev', amr_dev_raw, sent_vocab, tree_vocab)
  amr_train = utils.AMR('train', amr_train_raw, sent_vocab, tree_vocab)

  ### amr loader init
  train_dataloader = DataLoader(
    amr_train, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn)
  dev_dataloader = DataLoader(
    amr_dev, batch_size=args.batch_size, collate_fn=collate_fn)
  ...
  if args.resume_training and model_pt_file:
    ...
    if args.resume_training:
      print("[!] unable to find previous checkpoin, begin training from scratch")
  ...
  for i in range(args.epochs):
    ...
    if (i+1) % args.eval_every == 0:
      ...
      bleu = utils.raw_corpus_bleu(dev_preds_str, amr_dev_raw[1])
      ...
      if bleu > best_bleu:
        ...
        dev_preds_path = os.path.join(args.model_dir, C.GRAPH_PRED_FORMATTER.format('dev'))
        print("Exporting dev predictions at", dev_preds_path)
        utils.export_txt(dev_preds_str, dev_preds_path)
        try: # attempt to delete prvious checkpoint
      ...
      msg += "\n [SCORE] BLEU {:.2f} | ACC: {:.2f} | LENGTH {} toks".format(sample_bleu, sample_acc, len(sample_pred))
