...
def validate_vocab(vocab: C.STOI_T, is_target=False):
  """validates the integrity of vocab amr
...
def to_device(data: Iterable[torch.tensor], device: torch.device) -> Iterable[torch.tensor]:
  """moves incoming amr to the device"""
...
def export_vocabs(vocabs: C.VOCABS_T, out_dir: str):
  ...
  tree_vocab_path = os.path.join(out_dir, C.GRAPH_VOCAB_FILENAME)
  export_txt(tree_vocab[0], tree_vocab_path)

def export_amr_dataset(dataset: C.DATASET_T, out_dir: str, name: str):
  """exports raw amr dataset within out_dir"""
  assert name in C.AMR_SPLITS # sentinel
  ...
  tree_file = os.path.join(out_dir, C.GRAPH_DATA_FORMATTER.format(name))
...
def load_vocabs(data_dir) -> Tuple[C.VOCAB_T, C.VOCAB_T]:
  ...
  tree_vocab_path = os.path.join(data_dir, C.GRAPH_VOCAB_FILENAME)
  assert os.path.exists(tree_vocab_path)
  tree_vocab = load_vocab(tree_vocab_path)
  return sent_vocab, tree_vocab

def load_amr_dataset(data_dir, dataset_name: str) -> C.DATASET_T:
  """loads a raw amr dataset from data_dir"""
  ...
  tree_path = os.path.join(data_dir, C.GRAPH_DATA_FORMATTER.format(dataset_name))
...
if __name__ == '__main__':
    compute_smatch("/Users/jinzhao/Downloads/PA4_2023/outputs/processed_data/test_sent.txt", "/Users/jinzhao/Downloads/PA4_2023/outputs/model/test_graph_pred.txt")